...
create unique index if not exists emergency_reports_citizen_client_request_id_idx
  on public.emergency_reports (citizen_id, client_request_id)
...
begin
  ...
  if p_request_id is null then
    raise exception 'REPORT_REQUEST_ID_REQUIRED' using errcode = '22023';
  end if;

  perform pg_advisory_xact_lock(hashtext(actor_id::text || p_request_id::text));
...
revoke execute on function public.create_emergency_report(text, text, numeric, numeric, text) from authenticated;
revoke execute on function public.create_emergency_report(text, text, numeric, numeric, text, uuid) from public;
grant execute on function public.create_emergency_report(text, text, numeric, numeric, text, uuid) to authenticated;
